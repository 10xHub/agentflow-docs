...
type Logo = {
  icon: SimpleIcon;
  /** Display name override (some titles are awkward) */
  label?: string;
};

// Custom OpenAI SVG path (simple-icons removed it; this is the public OpenAI knot logo).
const customOpenAi: SimpleIcon = {
  title: 'OpenAI',
  slug: 'openai',
  hex: '412991',
    ...
    <section className={styles.wall} aria-label={heading}>
      ...
      <Marquee durationSec={durationSec}>
        {logos.map(({icon, label}) => (
          <div key={icon.slug} className={styles.logo} title={icon.title}>
            <svg
              role="img"
              viewBox="0 0 24 24"
